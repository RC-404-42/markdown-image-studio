...
1. 在電腦上解壓縮 `Markdown-Image-Studio-PWA-1.0.2-GitHub-Pages-FLAT.zip`。
2. 回到剛建立的 Repository。
...
取得新版部署包後，重複「上傳部署包內容」，允許 GitHub 覆蓋同名檔案並 Commit。重新開啟 Web App；若仍顯示舊版，先用 Safari 開啟原網址並在結尾加上 `?v=102`，確認新版載入後再完全關閉 Web App 並重開。請不要先清除網站資料，以免本機草稿一起消失。
